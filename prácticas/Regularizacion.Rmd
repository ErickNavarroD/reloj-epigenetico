...
```{r}
set.seed(1)
x = matrix(rnorm(100 * 20), 100, 20) # Muestrear una matriz de 20x100 a partir de una distribucion normal con media de 0 y SD de 1. 
...
predict(ridge.cv, newx , s = "lambda.1se", type = "response")  # make predictions
```
...
elastic.cv <- cv.glmnet(x,y,alpha=0.3,family="gaussian",type.measure = "mse", nfolds= 5 )
plot(elastic.cv)
